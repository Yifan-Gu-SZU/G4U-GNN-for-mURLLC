...
def build_graph_sequence(csis, norm_real, norm_imag, K):
    ...
    # Note that the directlink CSI for the previous frame will not be used by G4U, and it will be used for PG4U
...
class G4UConv(MessagePassing):
    def __init__(self, mlp1, mlp2, **kwargs):
        super(G4UConv, self).__init__(aggr='add', **kwargs)
        self.mlp1 = mlp1
        self.mlp2 = mlp2
    ...
    def message(self, x_i, x_j, edge_attr):
         # Use directlink CSI in the current frme and graph embedding for generating pilot beamformer
        tmp = x_j[:,2*Nt:]
        agg = self.mlp1(tmp)
...
class G4U(torch.nn.Module):
    def __init__(self):
        super(G4U, self).__init__()
        ...
        self.conv = G4UConv(self.mlp1,self.mlp2)

    def forward(self, data):
        hidden = Variable(torch.zeros(links*batches, self.hidden_size))
        hidden = hidden.to(device)
        outputs = torch.zeros(frames,links*batches,2*Nt)
...
def loss_and_QoS_evaluation(data, out, K):
    ...
    loss_output = torch.div(loss,frames)
    QoS_output = torch.div(QoS,frames)
...
# Remaining frame symbols for G4U by considering 
# overhead for CSI estimation and message passing
g4u_frame_symbols = max(0,total_frame_symbols - 2*train_K*Nt*O_csi-3*O_delay)
...
for epoch in range(1,51):
    ...
    frames = frame_num
    links = train_K
    frame_symbols = g4u_frame_symbols
    train_loss = train()
...
for test_K in gen_tests:
    ...
    g4u_frame_symbols = max(0,total_frame_symbols - 2*test_K*Nt*O_csi-3*O_delay)
